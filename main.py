...
def analyze_text(text):
    doc = nlp(text.lower())
    tokens = [token.lemma_ for token in doc if token.is_alpha and len(token) > 1]

    # Fallback: αν spaCy δεν δώσει tokens
    if not tokens:
        tokens = [w for w in re.findall(r'\w{2,}', text.lower())]
    ...
    for pattern in catastrophizing_patterns:
        if re.search(pattern, text.lower()): distortions.add("Καταστροφολογία")
    for pattern in overgeneralization_patterns:
        if re.search(pattern, text.lower()): distortions.add("Υπερ-γενίκευση")
    for pattern in personalization_patterns:
        if re.search(pattern, text.lower()): distortions.add("Προσωποποίηση")
